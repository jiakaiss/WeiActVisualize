...
### Requirement: 激活分布可视化
系统 SHALL 在「校准与激活」功能区展示选定 module 的激活分布，达到与权重分布视图同等的形态分析深度：全局激活直方图、per-token abs_max 分布的 violin（KDE + IQR + 中位数，离群 token 标记）、per-token 切片分布的 top-k by kurtosis violin 与 per-token 形态热力图。per-token 切片视图 SHALL 基于按需单 module 激活样本（模型加载后即可用，无需校准）；per-token abs_max 分布 violin 与离群标记 SHALL 依赖校准聚合的 per-token 统计。系统 SHALL 在未满足查看条件时给出明确提示，并 SHALL 移除冗余 / 过时的解释文本，仅保留有助于解读的术语说明。
...
- **THEN** 界面展示该 module 一次激活样本的 per-token 切片 violin（top-k by kurtosis）与 per-token 形态热力图，无需先运行校准
