...
- **per-token 切片分布视图（复用权重侧图表）**：按需捕获当前选中 module 的一次**输入**激活样本（复用 `capture_sample_inputs`），对每个 token 的 hidden 维分布计算形态指标（kurtosis / skewness / tail_ratio / shape_label），渲染 top-k by kurtosis 的 violin + per-token 形态热力图（复用 `channel_violin` / `channel_stats_heatmap`）。`[batch, seq, hidden]` 展平为 `[N, hidden]` 使每个 token 成为一个切片。这是「类似权重部分」的逐切片形态可视化在 per-token 粒度的落地。
- **分析输入而非输出**：W8A8 量化的是 Linear 输入 x（输出 y 在 fp32 累加且加权和趋向高斯、尾部轻），激活分析改为取 input role（校准聚合器本就同时采集 input/output，数据已在）。
...
- `interactive-visualization`：激活分布可视化升级到权重侧深度并改为分析**输入**激活--per-token 切片 violin（top-k by kurtosis，`[batch, seq, hidden]` 展平为 per-token 切片）+ per-token 形态热力图 + per-token abs_max 分布 violin + 离群幅度标记；移除 per-channel abs_mean 离群通道视图；并 SHALL 移除冗余 / 过时的解释文本。

## Impact

- **代码**：`stats/activation_stats.py`（`activation_stats_per_token` 补形态指标、新增 per-token 离群幅度函数）、`viz/charts.py`（per-token abs_max 分布 violin、复用 `channel_violin` / `channel_stats_heatmap`）、`viz/app.py`（`view_activation` 重构：per-token 切片视图 + 离群幅度 + 输入 role + 3D 展平 + 清理说明文本，复用 `capture_sample_inputs`）。
- **测试**：`tests/test_stats.py`（per-token 形态指标、离群幅度、3D 展平切片）、`tests/test_integration.py`（激活视图端到端）。
- **依赖**：无新增（plotly / numpy 已有）。
- **兼容性**：`view_activation` 返回值结构与 role（output->input）调整（UI 内部，非公开 API）；现有校准流程与在线聚合不变（input/output 仍都采集）。
