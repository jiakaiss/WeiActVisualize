...
- **不做**：per-channel（hidden 维）激活形态在线矩（保持现状的 SmoothQuant bar 诊断，不作主线）；激活量化前后对比；结果落盘缓存。
...
- `distribution-statistics`：分布形态指标与离群值检测扩展到 per-token 粒度——per-token 切片 SHALL 计算 kurtosis / skewness / tail_ratio / shape_label；per-token abs_max 分布 SHALL 量化离群 token 的比例、severity 与最大幅值。
- `interactive-visualization`：激活分布可视化升级到权重侧深度——per-token 切片 violin（top-k by kurtosis）+ per-token 形态热力图 + per-token abs_max 分布 violin + 离群幅度标记；并 SHALL 移除冗余 / 过时的解释文本。
- `model-tensor-capture`：新增按需单 module 激活样本捕获——为 per-token 切片视图提供一次前向的 output 激活样本，有界内存，仅对当前查看的 module 触发。
